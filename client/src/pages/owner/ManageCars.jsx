...
const deleteCar = async (carId)=>{
  try {
    ...
    const confirm = window.confirm('You Want to delete thiis car?')
...
useEffect(()=>{
 isOwner && fetchOwnerCars()
  ...
  return (
    ...
    <Title title='Manage Cars' subtitle='Organize, maintain, and track all cars in one place.Keep your fleet updated and well managed.'/>
      ...
      {cars.map((car, index)=>(
        ...
        <td className='p-3 flex items-center gap-3'> 
          ...
          <div className='max-md:hidden'>
            ...
            <p className='text-sm text-gray-600'>{car.seating_capacity} . {car.transimmion}</p>
          </div>
        </td>
        <td className='p-3 max-md:hidden'>{car.category}</td>
        <td className='p-3'>{currency}{car.price}/day</td>
